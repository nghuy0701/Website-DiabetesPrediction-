import { HealthData, PredictionResult, IHealthData, IPredictionResult } from '~/models'
...
const predictDiabetes = (input: PredictionInput): PredictionOutput => {
  const {
    pregnancies = 0,
    glucose,
    bloodPressure,
...
const generateRecommendations = (input: PredictionInput, riskLevel: string): string[] => {
...
const createPrediction = async (userId: string, inputData: PredictionInput): Promise<any> => {
  try {
...
const getUserPredictions = async (userId: string): Promise<any> => {
  try {
...
const getPredictionById = async (predictionId: string, userId: string): Promise<any> => {
  try {
    const prediction = await PredictionResult.findOne({
      _id: predictionId,
      userId
    }).populate('healthDataId')

    if (!prediction) {
      throw new ApiError(StatusCodes.NOT_FOUND, 'Không tìm thấy dự đoán')
    }
...
const getAllPredictions = async (): Promise<any> => {
  try {
...
const updatePrediction = async (
  ...
  try {
    let query: any = { _id: predictionId }
    ...
    if (updateData.reviewed && (userRole === 'doctor' || userRole === 'admin')) {
      prediction.reviewedBy = userId as any
...
const deletePrediction = async (predictionId: string, userId: string, userRole: string): Promise<void> => {
  try {
    let query: any = { _id: predictionId }
    ...
    const prediction = await PredictionResult.findOne(query)
    ...
    await PredictionResult.findOneAndDelete(query)

    // Also delete associated health data
    if (prediction.healthDataId) {
      await HealthData.findByIdAndDelete(prediction.healthDataId)
    }
  } catch (error: any) {
    throw new ApiError(StatusCodes.BAD_REQUEST, error.message)
  }
...
const getPredictionStatistics = async (userId: string): Promise<any> => {
  try {
    const predictions = await PredictionResult.find({ userId })
